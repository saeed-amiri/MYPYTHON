...
# PCL[0] = hexagon data list for transverse modes;
# PCL[1] = same for longitudinal modes
...
# choose your favorite colors: RdYlBu_r, jet, plasma, magma
  ...
  colors = palettable.cmocean.sequential.Thermal_5_r.mpl_colormap(np.linspace(0, 1, 256))
  ...
  for m in range(2):
    ...
    maxx = np.sort(PHAMP[i][m])[-6]
    med = np.median(PHAMP[i][m])
    print("max = ", maxx, " med =", med)

    # create the color array based on the cutoff
    clr=np.zeros(numpoints)
    n_top=n_max=n_med=n_down=0
    for k in range(numpoints):
      tmp=PHAMP[i][m][k]
      if tmp>=maxx : 
        clr[k]=1
        n_top+=1
      elif 0.01*maxx<=tmp<maxx : 
        clr[k]=0.9
        n_max+=1
      elif 100*med<=tmp<0.01*maxx : 
        clr[k]=0.5
        n_med+=1
      else : 
        clr[k]=0.00
        n_down+=1
    print('n_top ', n_top, 'n_max ',n_max, 'n_med ', n_med,'n_down ',n_down)
