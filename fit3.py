...
with PdfPages(OutDir+'/df.pdf') as pdf:
	class DATA():
		def __init__(self):
			...
			t0 = []; t1 = []; kick=[]; coef1 = []; eta1 = []; coef2 = []; eta2 = []; thermalized = []; vx0 = []; vy0 = []
			for i,dir in enumerate(df.DIR):
				dj=JOB(dir)
				t0.append(dj.crit_place); t1.append(dj.stop); kick.append(dj.kick); thermalized.append(dj.thermalized)
				vx0.append(dj.vx0); vy0.append(dj.vy0)
				#fitting with function a*exp(x)
				p,_ = curve_fit(func1,dj.argdecay,dj.decay,p0=[0.01,0.0001])
				coef1.append(p[0]);	eta1.append(p[1])
				#fitting with function a*exp(x^2)
				p,_ = curve_fit(func2,dj.argdecay,dj.decay,p0=[0.01,0.0001])
				coef2.append(p[0]);	eta2.append(p[1])
				if i == 0 : init=INIT(dir)
			...
			axes[0].axis('off')
			axes[0].set_title('Directory infos:',x=0, y=1,ha='left',fontsize=13)
			axes[0].text(0,0.95, f'Parent dir: {os.getcwd()}')
			axes[0].text(0,0.9, f'Source dir: {init.cwd}')
			axes[0].text(0,0.85, f'Numbers of sub-directories: {len(kick)}')
			if err_dirs:
				for ii, (ed, ee) in enumerate(zip(err_dirs,err)):
					axes[0].text(0,0.75-ii*0.1,f'err_dirs: {ed}\nerr: {ee}')
			axes[1].axis('off')
			axes[1].set_title('Simulations infos:',x=0,y=1,ha='left',fontsize=13)
			axes[1].text(0,0.95, f'Bondstyle: {init.bondstyle}',color = 'r')
			axes[1].text(0,0.9, r'$L_x/L_y =$ {}'.format(init.lx/init.ly))
			axes[1].text(0,0.85, f'Numbers of particles = {init.natoms}')
			axes[1].text(0,0.8, f'Numbers of bonds = {init.nbonds}')
			axes[1].text(0,0.75, f'Coupling = {init.coupling}')
			axes[1].text(0,0.7, f'Cut bonds: {init.cutbonds}')
			axes[1].text(0,0.65, f'Timestep = {init.timestep}')
			###################################################################################################
