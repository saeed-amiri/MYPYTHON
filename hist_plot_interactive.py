...
#os.system("read_lammps_LOG.py LOG.{} 0 C_VEL[1] C_VEL[2]".format(RUNID))
vfile="SLOG."+RUNID
...
def set_color(i):
  ...
  for m in range(2):
    ...
    colors1 = palettable.cmocean.sequential.Gray_10_r.mpl_colormap(np.linspace(0,0.5,256))
...
def histogram(m):
  ...
  glevel = int(numpoints/Thold)
  tmp = np.sort(PHAMP[INDEX][m])
  maxpoint = tmp[-1:]

  red_index = [ i for i, j in enumerate(tmp) if j == maxpoint]
  red_len = len(red_index)
  red_point = tmp[-(red_len):]

  yellow = tmp[-(red_len+20)]
  yellow_index = [ i for i, j in enumerate(tmp[:-(red_len)]) if j >= yellow ]
  yellow_len = len(yellow_index)
  yellow_point = tmp[-(yellow_len):-red_len]

  histlevel = tmp[-glevel:-(yellow_len)]
  histmp = tmp[:-glevel]
  histlist=[histmp, histlevel, yellow_point, red_point]
  colors=['gainsboro', 'dimgrey', 'y', 'r']
  ...
  ax.hist(histlist, bins=50, histtype='stepfilled',
          alpha=0.5,log=True, color=colors)
...
def on_keyboard(event):
  ...
  for m in range(2):
    ...
    plt.sca(ax) 
    s='TA' if m==0 else 'LA'
    tmp = '{}, F={} MEDIAN={:.3E}'.format(s,INDEX,qq[m])
